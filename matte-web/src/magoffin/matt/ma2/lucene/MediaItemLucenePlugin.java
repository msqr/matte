...
public class MediaItemLucenePlugin extends AbstractLucenePlugin {
	...
	public SearchMatch build(Document doc) {
		...
		searchResult.setWidth(Integer.parseInt(doc.get(IndexField.MEDIA_WIDTH.getFieldName())));
		searchResult.setHeight(Integer.parseInt(doc.get(IndexField.MEDIA_HEIGHT.getFieldName())));
		...
		String[] sharedAlbumKeys = doc.getValues(IndexField.MEDIA_SHARED_ALBUM_KEY.getFieldName());
		if ( sharedAlbumKeys != null ) {
			String[] sharedAlbumNames = doc.getValues(IndexField.MEDIA_SHARED_ALBUM_NAME.getFieldName());
	...
	public Object getNativeQuery(SearchCriteria criteria) {
		...
		if ( itemCriteria.isSharedOnly() ) {
			root.add(new TermQuery(new Term(IndexField.MEDIA_SHARED_FLAG.getFieldName(), "1")),
					Occur.MUST);
		}
		...
		Query dateRangeQuery = getDateRangeQuery(IndexField.ITEM_DATE, IndexField.ITEM_DATE_MONTH,
				itemCriteria);
		...
		if ( StringUtils.hasText(itemCriteria.getQuickSearch()) ) {
			should.add(getLucene().parseQuery(getIndexType(), itemCriteria.getQuickSearch()),
					Occur.SHOULD);
		}
		if ( template != null && template.getUserTag().size() > 0 ) {
			for ( UserTag tag : (List<UserTag>) template.getUserTag() ) {
				...
				getLucene().addTokenizedTermQuery(should, tag.getTag(), tagField, getIndexType());
			}
		}
	...
	private List<Object> indexMediaItem(MediaItem item, IndexWriter writer) {
		...
		if ( log.isDebugEnabled() ) {
			log.debug("Indexing MediaItem " + item.getItemId() + " (" + item.getPath() + ")");
		}
		...
		if ( item.getName() != null ) {
			doc.add(new Field(IndexField.ITEM_NAME.getFieldName(), item.getName(), Field.Store.YES,
		...
		if ( item.getCreationDate() != null ) {
			...
			doc.add(new Field(IndexField.CREATED_DATE_MONTH.getFieldName(), monthStr, Field.Store.NO,
					Field.Index.NOT_ANALYZED));
		}
		...
		if ( itemDate != null ) {
			...
			if ( dateStr != null ) {
				doc.add(new Field(IndexField.ITEM_DATE.getFieldName(), dateStr, Field.Store.YES,
						Field.Index.NOT_ANALYZED));
				doc.add(new Field(IndexField.ITEM_DATE_MONTH.getFieldName(), monthStr, Field.Store.NO,
						Field.Index.NOT_ANALYZED));
				doc.add(new Field(IndexField.ITEM_DATE_TIME_ZONE.getFieldName(), tz.getID(),
						Field.Store.YES, Field.Index.NO));
			}
		}

		doc.add(new Field(IndexField.MEDIA_MIME.getFieldName(), item.getMime(), Field.Store.YES,
				Field.Index.ANALYZED));
		generalText.append(item.getMime()).append(" ");

		doc.add(new Field(IndexField.MEDIA_HEIGHT.getFieldName(), String.valueOf(item.getHeight()),
				Field.Store.YES, Field.Index.NO));

		doc.add(new Field(IndexField.MEDIA_WIDTH.getFieldName(), String.valueOf(item.getWidth()),
				Field.Store.YES, Field.Index.NO));

		// metadata
		List<Metadata> metadata = item.getMetadata();
		for ( Metadata meta : metadata ) {
			doc.add(new Field(IndexField.METADATA.getFieldName(), meta.getKey() + ':' + meta.getValue(),
					Field.Store.YES, Field.Index.NOT_ANALYZED));
		...
		for ( UserTag tag : tags ) {
			doc.add(new Field(IndexField.TAG.getFieldName(), tag.getTag(), Field.Store.YES,
					Field.Index.ANALYZED));

			if ( tag.getTaggingUser() != null ) {
				doc.add(new Field(getUserTagFieldName(tag), tag.getTag(), Field.Store.YES,
						Field.Index.ANALYZED));
			}
			generalText.append(tag.getTag()).append(" ");
		}

		List<Album> sharedAlbums = albumDao.findSharedAlbumsContainingItem(item);
		for ( Album album : sharedAlbums ) {
			doc.add(new Field(IndexField.MEDIA_SHARED_ALBUM_KEY.getFieldName(), album.getAnonymousKey(),
					Field.Store.YES, Field.Index.NOT_ANALYZED));
		...
		doc.add(new Field(IndexField.ITEM_OWNER.getFieldName(), c.getOwner().getUserId().toString(),
		...
		for ( MediaItemRating rating : ratings ) {
			doc.add(new Field(getUserRatingFieldName(rating), String.valueOf(rating.getRating()),
					Field.Store.YES, Field.Index.NOT_ANALYZED));
	...
	private final class MediaItemIndexResultsCallback extends
		...
		protected BatchCallbackResult doHandle(MediaItem item) throws Exception {
			...
			if ( log.isInfoEnabled() && (getNumProcessed() % getInfoReindexCount()) == 1 ) {
				log.info("Indexing MediaItem row " + (getNumProcessed() + 1) + " {itemId="
