 ...
 * 
 * <p>
 * <b>Known limitations:</b> non-ASCII characters in file paths are not handled
 * well in some situations. In this case the files might not be copied, and the
 * <code>metadata.xml</code> for the album will be created but may have
 * incorrect archive paths set.
 * </p>
 * 
...
public class MediaAlbumExportTool {
	...
	private final String ITEMS_FOR_ALBUM_SQL = "select item.path, item.name, item.comment, item.hits, item.creation_date, "
	...
	public void run() {
		...
		List<Map<String, Object>> albums = mediaAlbumJdbcTemplate.queryForList(TOP_LEVEL_ALBUM_SQL,
				new Object[] { ownerId });
		int largestId = 0;
	...
	private File exportAlbum(File dir, Map<String, ?> albumData, CollectionImport xml,
			AlbumImportType parent) throws JAXBException, IOException {
		...
		if ( log.isInfoEnabled() ) {
			log.info("Exporting album [" + albumName + "], ID " + albumId + " to " + albumDir);
		}
		...
		List<Map<String, Object>> childAlbums = mediaAlbumJdbcTemplate.queryForList(CHILD_ALBUM_SQL,
				new Object[] { albumId });
	...
	@SuppressWarnings("unchecked")
	private void exportAlbumItems(Integer albumId, AlbumImportType albumXml, File albumDir)
			throws JAXBException, IOException {
		List<Map<String, Object>> items = mediaAlbumJdbcTemplate.queryForList(ITEMS_FOR_ALBUM_SQL,
				new Object[] { albumId, ownerId, ownerId });
		for ( Map<String, ?> itemData : items ) {
			...
			File itemFile = new File(collectionDir, itemData.get("cpath") + itemPath);
			...
			if ( itemFile.exists()
					&& (!outFile.exists() || (outFile.exists() && itemFile.length() != outFile.length())) ) {
			...
			String archivePath = itemResource.getFile().getAbsolutePath()
					.substring(basePath.length() + 1);
			itemXml.setArchivePath(archivePath);
			albumXml.getItem().add(itemXml);
		}
		if ( log.isInfoEnabled() ) {
			log.info("Exported " + items.size() + " items for album [" + albumXml.getName() + "]");
		}
	}
	...
	public static void main(String[] args) {
		String configLocation = args != null && args.length > 0 ? args[0]
				: "MediaAlbumExportToolContext.xml";
		ClassPathXmlApplicationContext context = null;
		try {
			context = new ClassPathXmlApplicationContext(configLocation);
			MediaAlbumExportTool tool = context.getBean("mediaAlbumExportTool",
					MediaAlbumExportTool.class);
			tool.run();
		} finally {
			if ( context != null ) {
				context.close();
