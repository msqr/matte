...
create table album (AlbumId int8 not null, Hjtype varchar(255) not null, Theme int8, Owner_ int8, Comment_ varchar(2048), AllowFeed bool not null, Poster int8, AllowAnonymous bool not null, AnonymousKey varchar(96), AllowOriginal bool not null, AlbumDate timestamp, SortMode int4, Name_ varchar(255) not null, AllowBrowse bool not null, ModifyDate timestamp, CreationDate timestamp not null, Album_Album_Hjid int8, Album_Album_Hjindex int4, primary key (AlbumId));
create table collection (CollectionId int8 not null, Hjtype varchar(255) not null, Owner_ int8, Comment_ varchar(2048), Path_ varchar(255) not null, Name_ varchar(128) not null, ModifyDate timestamp, CreationDate timestamp not null, primary key (CollectionId));
create table media_item (ItemId int8 not null, Hjtype varchar(255) not null, Hits int4 not null, Height int4 not null, TzDisplay varchar(40) not null, MediaType int8, Description varchar(2048), Tz varchar(40) not null, Width int4 not null, Path_ varchar(255) not null, Mime varchar(64) not null, DisplayOrder int4 not null, Name_ varchar(128) not null, UseIcon bool not null, ModifyDate timestamp, CreationDate timestamp not null, FileSize int8 not null, ItemDate timestamp, Collection_Item_Hjid int8, Collection_Item_Hjindex int4, primary key (ItemId));
create table media_item_rating (RatingId int8 not null, Hjtype varchar(255) not null, RatingUser int8, Rating int2 not null, CreationDate timestamp not null, MediaItem_UserRating_Hjid int8, MediaItem_UserRating_Hjindex int4, primary key (RatingId));
...
create table theme (ThemeId int8 not null, Hjtype varchar(255) not null, Owner_ int8, BasePath varchar(255) not null, Description varchar(2048), Author varchar(64), Name_ varchar(128) not null unique, ModifyDate timestamp, AuthorEmail varchar(128), CreationDate timestamp not null, primary key (ThemeId));
...
create table user_comment (CommentId int8 not null, Hjtype varchar(255) not null, CommentingUser int8, Comment_ varchar(2048) not null, Approved bool, CreationDate timestamp not null, Commenter varchar(255), MediaItem_UserComment_Hjid int8, MediaItem_UserComment_Hjindex int4, primary key (CommentId));
create table user_tag (TagId int8 not null, Hjtype varchar(255) not null, Tag varchar(2048) not null, TaggingUser int8 not null, CreationDate timestamp not null, MediaItem_UserTag_Hjid int8, MediaItem_UserTag_Hjindex int4, primary key (TagId));
