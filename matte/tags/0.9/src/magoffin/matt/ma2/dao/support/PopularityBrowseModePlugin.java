 ...
 *   into index keys. The SQL must return two columns: an integer {@code bucket} 
 *   for the lower bound of the bucket range, and an integer {@code item_count}
 *   for the count of items that fall within this bucket.</p>
 *   
 *   <p>The SQL query takes a template parameter {@code 0} as the bucket size, 
 *   configured by the {@link #setPopularityBucketSize(int)} property. Then the 
 *   following positional SQL parameters will be set:</p>
 *   
 *   <ol>
 ...
 *   popularity bucket range) and also to populate the items in a single album
...
public class PopularityBrowseModePlugin extends AbstractJdbcBrowseModePlugin {
	...
	public SearchResults find(BrowseAlbumsCommand command,
		...
		if ( m.matches() ) {
			...
			populateAlbums(user.getUserId(), albums, 
					command.getLocale(), bucketKey, pageOffset);
		} else {
			handleSearchForPopularity(user.getUserId(), 
	...
	private void populateAlbums(final Long userId,
			...
			final int sectionBucket, final int singleAlbumOffset) {
		getJdbcTemplate().query(new PreparedStatementCreator() {
			public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
				if ( log.isDebugEnabled() ) {
					log.debug("Popularity section search with SQL ["
							+sqlBrowseSection +"]");
				}
	...
	private void setAlbumInfo(List<AlbumSearchResult> albums, 
			AlbumSearchResult album, int albumItemCount, 
			int bucket, Locale locale ) {
		...
		Object[] args = new Object[] {lowerBound, upperBoundInclusive, upperBound, 
				startPos, endPos};
		if ( albumItemCount == 1 ) {
	...
	/**
	 * @param popularityBucketSize the popularityBucketSize to set
	 */
