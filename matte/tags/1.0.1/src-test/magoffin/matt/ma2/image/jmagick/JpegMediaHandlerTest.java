...
/**
 * Test case for the {@link magoffin.matt.ma2.image.jmagick.JpegMediaHandler}
 * class.
 * 
...
public class JpegMediaHandlerTest extends AbstractSpringEnabledTransactionalTest {
	...
	/**
	 * Test JPEG2000 support.
	 * 
	 * @throws Exception if an error occurs
	 */
	public void testJpeg2000Support() throws Exception {
		
		JpegMediaHandler handler = new JpegMediaHandler();
		handler.setDomainObjectFactory(testJpegMediaHandler.getDomainObjectFactory());
		handler.setJmagickMediaEffectMap(testJpegMediaHandler.getJmagickMediaEffectMap());
		handler.setMediaBiz(testJpegMediaHandler.getMediaBiz());
		handler.setUseJpeg2000(true);
		...
		MediaItem item = handler.createNewMediaItem(tmpFile);
		item.setPath(tmpFile.getName());
		...
		for ( MediaSize size : sizeSet ) {
			for ( MediaQuality quality : qualitySet ) {
				File tmpOutputFile = File.createTempFile("JpegMediaHandlerTest-", ".jp2");
				BasicMediaRequest request = new BasicMediaRequest(null, size, quality);
				request.getParameters().put(MediaRequest.OUTPUT_FILE_KEY, tmpOutputFile);
				BasicMediaResponse response = new BasicMediaResponse(
						new FileOutputStream(tmpOutputFile));
				...
				assertTrue(tmpOutputFile.length() > 0);
				if ( logger.isDebugEnabled() ) {
					String msg = "Created JPEG2000 size " +size +" quality [" +quality +"] in "
						+time +"ms: " +tmpOutputFile.getAbsolutePath();
					logger.debug(msg);
					outMsg.append(msg).append("\n");
				}
