...
//	@javax.annotation.Resource private CollectionDao collectionDao;
	...
	public void testRegisterAndConfirmUser() throws Exception {
		...
		assertNotNull("The confirmed user must have a thumbnail spec", confirmedUser.getThumbnailSetting());
	...
	public void testRegisterBadConfirmation() throws Exception {
		...
		try {
			@SuppressWarnings("unused") User confirmedUser = testUserBizImpl.confirmRegisteredUser(
					newUser.getLogin(), "this is not the confirmation code",context);
			fail("AuthorizationException should have been thrown");
		} catch (AuthorizationException e) {
			assertTrue("AuthroizationException with reason [" 
					+AuthorizationException.Reason.REGISTRATION_NOT_CONFIRMED +"] should have been thrown",
	...
	public void testDoNotChangePassword() throws Exception {
		...
		assertEquals("The resaved user's password should not be changed", 
				originalPassword, savedUser.getPassword());
		assertEquals("The resaved user's name should be updated",
	...
	public void testForgotPasswordAndConfirm() throws Exception {
		...
		User confirmedUser = testUserBizImpl.confirmForgotPassword(
				newUser.getLogin(), confCode, 
