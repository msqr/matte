/* ===================================================================
 * GenericVideoMediaHandlerTest.java
 * 
 * Created Jan 12, 2007 3:39:08 PM
 * 
...
public class GenericVideoMediaHandlerTest extends
	...
	/** The video media handler to test. */
	protected MediaHandler testVideoMediaHandler;
	
	/**
	 * Test the media handler can correctly read video dimensions.
	 * @throws IOException if an error occurs
	 */
	public void testReadDimensions() throws IOException {
		Resource testReadDimensions 
			= new ClassPathResource("magoffin/matt/ma2/video/MVI_1586.AVI");
		MediaItem item = testVideoMediaHandler.createNewMediaItem(
				testReadDimensions.getFile());
		assertNotNull(item);
		assertEquals(320,item.getWidth());
		...
		assertEquals(VideoConstants.AVI_MIME, item.getMime());
	}
	...
	public void testAllSizesAndQualities() throws Exception {
		...
		File tmpFile = File.createTempFile("GenericVideoMediaHandlerTest-input-", 
				".AVI", new File("/tmp"));
		ClassPathResource cpResource = new ClassPathResource(
				"magoffin/matt/ma2/video/MVI_1586.AVI");
		FileCopyUtils.copy(cpResource.getFile(), tmpFile);
		
		MediaItem item = testVideoMediaHandler.createNewMediaItem(tmpFile);
		...
		for ( MediaSize size : sizeSet ) {
			for ( MediaQuality quality : qualitySet ) {
				File tmpOutputFile = File.createTempFile("GenericVideoMediaHandlerTest-", ".jpg");
				...
				testVideoMediaHandler.handleMediaRequest(item,  request, response);
