...
public class LuceneBizTest extends AbstractSpringEnabledTransactionalTest {
	...
	@Resource
	private LuceneBiz testLuceneBiz;
	@Resource
	private UserBiz testUserBiz;
	@Resource
	private MediaBiz testMediaBiz;
	@Resource
	...
	private final IndexUpdateTracker updateTracker = new IndexUpdateTracker();
		...
		IndexListener listener = new IndexListener() {
			...
			public void onIndexEvent(IndexEvent event) {
				if ( event.getType() == EventType.UPDATE
						&& IndexType.USER.name().equals(event.getIndexType()) ) {
	...
	public void testFindUsersForIndex() throws Exception {
		...
		PaginationIndexSection indexA = (PaginationIndexSection) results.getIndex().getIndexSection()
				.get(0);
		assertEquals("a", indexA.getIndexKey());
		assertEquals(2, indexA.getCount());

		PaginationIndexSection indexZ = (PaginationIndexSection) results.getIndex().getIndexSection()
				.get(1);
	...
	public void testIndexAllMediaItems() throws Exception {
		...
		importImage("magoffin/matt/ma2/image/bee-action.jpg", collectionDao, testIOBizImpl,
				testCollection, testCollection.getOwner());
		importImage("magoffin/matt/ma2/image/dylan2.jpg", collectionDao, testIOBizImpl, testCollection,
				testCollection.getOwner());
		importImage("magoffin/matt/ma2/image/IMG_4215.jpg", collectionDao, testIOBizImpl, testCollection,
				testCollection.getOwner());
		...
		IndexListener listener = new IndexListener() {
			...
			public void onIndexEvent(IndexEvent event) {
				if ( event.getType() == EventType.UPDATE
						&& IndexType.MEDIA_ITEM.name().equals(event.getIndexType()) ) {
	...
	public void testFindMediaItemById() throws Exception {
		...
		SearchResults results = testLuceneBiz.findMediaItems(criteria, null, myBizContext);
		assertNotNull(results);
	...
	public void testFindMediaItemByTag() throws Exception {
		...
		assertEquals(myBizContext.getActingUser().getUserId(), foundTag.getTaggingUser().getUserId());
	...
	private Collection setupTestCollection() {
		...
		List<Collection> collections = testUserBiz.getCollectionsForUser(testUser, myBizContext);
	...
	private User registerAndConfirmUser(String login, String email) throws AuthorizationException {
		...
		User confirmedUser = testUserBiz.confirmRegisteredUser(newUser.getLogin(), confKey,
				myBizContext);
