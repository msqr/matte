...
/**
 * Extension of
 * {@link org.springframework.ws.soap.axiom.AxiomSoapMessageFactory} that allows
 * configuring the XMLInputFactory properties, to allow for handling very large
 * element content by turning off text coalescing.
 * 
 * <p>
 * See <a href="https://jira.springframework.org/browse/SWS-650">SWS-650</a>.
 * </p>
 * 
 * @version 1.1
 */
public class AxiomSoapMessageFactory implements SoapMessageFactory, InitializingBean {
	...
	private Map<String, Object> inputFactoryProperties;
	...
	/**
	 * Indicates whether the SOAP Body payload should be cached or not. Default
	 * is <code>true</code>.
	 * <p/>
	 * Setting this to <code>false</code> will increase performance, but also
	 * result in the fact that the message payload can only be read once.
	 * 
	 * @param payloadCaching
	 *        the value
	 */
	 ...
	 * <code>false</code>.
	 * <p/>
	 * Setting this to <code>true</code> will cause Axiom to store larger
	 * attachments on disk, rather than in memory. This decreases memory
	 * consumption, but decreases performance.
	 * 
	 * @param attachmentCaching
	 *        the value
	 */
	 ...
	 * {@link #setAttachmentCaching(boolean) attachmentCaching} is set to
	 * <code>true</code>.
	 * <p/>
	 * The parameter should be an existing, writable directory. This property
	 * defaults to the temporary directory of the operating system (i.e. the
	 * value of the <code>java.io.tmpdir</code> system property).
	 * 
	 * @param attachmentCacheDir
	 *        the value
	 */
	...
	/**
	 * Sets the threshold for attachments caching, in bytes. Attachments larger
	 * than this threshold will be cached in the
	 * {@link #setAttachmentCacheDir(File) attachment cache directory}. Only
	 * used when {@link #setAttachmentCaching(boolean) attachmentCaching} is set
	 * to <code>true</code>.
	 * <p/>
	 * Defaults to 4096 bytes (i.e. 4 kilobytes).
	 * 
	 * @param attachmentCacheThreshold
	 *        the value
	 */
	 ...
	 * <p/>
	 * The default is {@code true}, to comply with WS-I, but this flag can be
	 * set to {@code false} to the older W3C SOAP 1.1 specification.
	 * 
	 * @param langAttributeOnSoap11FaultString
	 *        the value
	 *
	 * @see <a href=
	 *      "http://www.ws-i.org/Profiles/BasicProfile-1.1.html#SOAP_Fault_Language">WS-I
	 *      Basic Profile 1.1</a>
	 */
	...
	public void afterPropertiesSet() throws Exception {
		...
		if ( inputFactoryProperties != null ) {
			for ( Map.Entry<String, Object> entry : inputFactoryProperties.entrySet() ) {
	...
	public WebServiceMessage createWebServiceMessage(InputStream inputStream) throws IOException {
		...
		try {
			...
			throw new AxiomSoapMessageCreationException("Could not parse request: " + ex.getMessage(),
					ex);
		} catch ( OMException ex ) {
			throw new AxiomSoapMessageCreationException("Could not create message: " + ex.getMessage(),
					ex);
		}
	}
	...
	/**
	 * @param inputFactoryProperties
	 *        the inputFactoryProperties to set
	 */
	public void setInputFactoryProperties(Map<String, Object> inputFactoryProperties) {
		this.inputFactoryProperties = inputFactoryProperties;
	}
