 ...
 * otherwise an icon is used. For icon resolution, the item's MIME type will be
 * used to search for a PNG image icon using the
 * {@link #getIconResourcePathPrefix()} path as the prefix, <code>.png</code> as
 * a suffix, and the following search paths will be tried:
 * </p>
 * 
 * <ol>
 * <li>mime</li>
 ...
 * 
 * <p>
 * For example, for an item with a file name <code>my-video.mpg</code> and with
 * a MIME type of <code>video/mpeg</code> and the default
 * <code>iconResourcePathPrefix</code>, the following paths will be tried:
 * </p>
 * 
 * <ol>
 ...
 * <dd>A {@link MediaHandler} implementation to handle the embedded image of the
 * metadata, if available.</dd>
 * 
 * <dt>iconResourcePathPrefix</dt>
 * <dd>A resource prefix path to use for resolving icon images.</dd>
 * 
 * <dt>resourceLoader</dt>
 * <dd>A {@link ResourceLoader} to use for loading icon resources. Defaults to a
 * {@link DefaultResourceLoader} instance.</dd>
 * 
...
public class BasicIconBasedMediaHandler extends AbstractMediaHandler {
	...
	private final Map<String, byte[]> iconCache = new WeakHashMap<String, byte[]>();
	...
	public String getFileExtension(MediaItem item, MediaRequest request) {
		...
		if ( !(metadata instanceof EmbeddedImageMetadata) ) {
			throw new RuntimeException("EmbeddedImageMetadata expected, but have [" + metadata + "]");
		}
		...
		return getImageMediaRequestDelegate().getDelegateFileExtension(itemResource, embedMime, item,
				request);
	}
	...
	protected void defaultHandleRequest(MediaItem item, MediaRequest request, MediaResponse response) {
		...
		EmbeddedImageMetadata imageMeta = (EmbeddedImageMetadata) metadata;
		getImageMediaRequestDelegate().handleDelegateMediaRequest(imageMeta.getEmbeddedImageResource(),
				imageMeta.getEmbeddedImageMimeType(), item, request, response);
	}
	 ...
	 *        the response
	 */
	protected void defaultHandleIconResponse(MediaItem item, MediaRequest request,
	...
	/**
	 * @param imageMediaRequestDelegate
	 *        the imageMediaRequestDelegate to set
	 */
	...
	/**
	 * @param iconResourcePathPrefix
	 *        the iconResourcePathPrefix to set
	 */
	...
	/**
	 * @param fallbackIconName
	 *        the fallbackIconName to set
	 */
