 ...
 * of all browsable items, grouped by average user ratings each item has, e.g.
 * "0.5", "1", etc. These "buckets" will be turned into index keys. The SQL must
 * return two columns: a float {@code avg_rating} for the average rating bucket,
 * and an integer {@code item_count} for the count of items that fall within
 * this bucket.
 * </p>
 * 
 * <p>
 ...
 * <dd>
 * <p>
 * The SQL to generate the results for a single index section (i.e. average
 * rating) and also to populate the items in a single album for viewing a single
 * album. The following positional SQL parameters will be set:
 * </p>
 * 
 * <ol>
 * <li>User ID (long) - the user to display the browsable items for</li>
 ...
 * 
 * <p>
 * The average rating section is set twice to allow the SQL to set lower and
 * upper bounds for that section.
 * </p>
 * 
 * <p>
...
public class RatingAverageBrowseModePlugin extends AbstractJdbcBrowseModePlugin {
	...
	public static final String MESSAGE_KEY_ALBUM_TITLE_SINGLE = "rating.album.single.title";
	...
	public SearchResults find(BrowseAlbumsCommand command, PaginationCriteria pagination) {
		...
		if ( m.matches() ) {
			...
			populateAlbums(user.getUserId(), albums, command.getLocale(), m.group(1), avgRatingKey,
					pageOffset);
		} else {
			handleSearchForAverageRating(user.getUserId(), pagination, albums, index,
					command.getLocale());
		}
	...
	private void populateAlbums(final Long userId, final List<AlbumSearchResult> albums,
			final Locale locale, final String sectionKey, final float sectionRating,
