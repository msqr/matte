...
public class RotateEffect extends BaseAwtMediaEffect {
	...
	public BufferedImage applyEffect(MediaItem item, MediaRequest request, BufferedImage source) {
		...
		if ( degrees == -90 ) {
			xform.preConcatenate(AffineTransform.getTranslateInstance(0, source.getWidth()));
		} else if ( degrees == 90 ) {
			xform.preConcatenate(AffineTransform.getTranslateInstance(source.getHeight(), 0));
		} else {
			xform.preConcatenate(AffineTransform.getTranslateInstance(0, source.getHeight()));
			xform.preConcatenate(AffineTransform.getTranslateInstance(source.getWidth(), 0));
		...
		if ( degrees != 180 ) {
			newImage = new BufferedImage(source.getHeight(), source.getWidth(), source.getType());
		} else {
			newImage = new BufferedImage(source.getWidth(), source.getHeight(), source.getType());
		}
