 ...
 * 
 * <p>
 * Different scaling algorithms can be specified for thumbnail and non-thumbnail
 * sized images, with the idea that thumbnails could use a faster (but uglier)
 * algorithm while non-thumbnails could use a slower (but prettier) one. By
 * default the thumbnail algorithm is set to <b>Lanczos</b> and the
 * non-thumbnail algorithm is <b>Sinc</b>.
 * </p>
 * 
 * <p>
 ...
 * <dt>filterSet</dt>
 * <dd>A set of allowed filter names. This has a default value containing the
 * filters supported by GraphicsMagick.</dd>
 * 
 ...
 * <dd>The filter name to use for non-thumbnail-sized images. Defaults to
 * {@link #DEFAULT_NORMAL_FILTER_NAME}.</dd>
 * 
 * <dt>filterThumbnailSizes</dt>
 * <dd>A set of {@link MediaSize} for which to treat as thumbnail sizes and
 * scale with the <code>thumbnailFilterName</code> as opposed to the
 * <code>normalFilterName</code>. Defaults to a set containing
 * {@link MediaSize#THUMB_SMALL}, {@link MediaSize#THUMB_NORMAL}, and
 * {@link MediaSize#THUMB_BIG}. The {@link MediaSize#THUMB_BIGGER} is
 * intentionally left out so that the <code>normalFilterName</code> will be used
 * for this size to achieve higher quality.</dd>
 * 
 * <dt>thumbnailUnsharp</dt>
 * <dd>Parameters to use for applying an unsharp operation after the scale of
 * thumbnail images, to achieve a higher quality looking image. Set to
 * <em>null</em> to not apply an unsharp operation to thumbnails. Accepts up to
 * 4 values, for <em>radius, sigma, amount, and threshold</em>. Defaults to
 * array with two values: <code>0,1</code>.</dd>
 * 
 * <dt>normalUnsharp</dt>
 * <dd>Parameters to use for applying an unsharp operation after the scale of
 * normal images, to achieve a higher quality looking image. Set to
 * <em>null</em> to not apply an unsharp operation to normal images. Accepts up
 * to 4 values, for <em>radius, sigma, amount, and threshold</em>. Defaults to
 * array with two values: <code>0,1</code>.</dd>
 * 
 * <dt>unsharpThumbnailSizes</dt>
 * <dd>A set of {@link MediaSize} for which to treat as thumbnail sizes and
 * unsharp with the <code>thumbnailUnsharp</code> as opposed to the
 * <code>normalUnsharp</code>. Defaults to a set containing
 * {@link MediaSize#THUMB_SMALL}, {@link MediaSize#THUMB_NORMAL}, and
 * {@link MediaSize#THUMB_BIG} and {@link MediaSize#THUMB_BIGGER}.</dd>
 * 
 * </dl>
 * 
 * @author matt
...
public class ScaleEffect extends BaseIM4JavaMediaEffect {
	...
	private Set<MediaSize> filterThumbnailSizes = EnumSet.of(MediaSize.THUMB_BIG, MediaSize.THUMB_NORMAL,
	...
	private Set<MediaSize> unsharpThumbnailSizes = EnumSet.of(MediaSize.THUMB_BIGGER,
	...
	public ImageCommandAndOperation applyEffect(MediaItem item, MediaRequest request,
		...
		String filterName = filterThumbnailSizes.contains(request.getSize()) ? thumbnailFilterName
				: normalFilterName;
		if ( filterName != null ) {
		...
		Double[] unsharp = unsharpThumbnailSizes.contains(request.getSize()) ? thumbnailUnsharp
				: normalUnsharp;
	...
	/**
	 * @param thumbnailFilterName
	 *        the thumbnailFilterName to set
	 */
	public void setThumbnailFilterName(String thumbnailFilterName) {
		if ( filterSet != null && !filterSet.contains(thumbnailFilterName) ) {
			throw new IllegalArgumentException(
					"[" + thumbnailFilterName + "] is not a supported filter");
		}
	...
	/**
	 * @param normalFilterName
	 *        the normalFilterName to set
	 */
	public void setNormalFilterName(String normalFilterName) {
		if ( filterSet != null && !filterSet.contains(normalFilterName) ) {
			throw new IllegalArgumentException("[" + normalFilterName + "] is not a supported filter");
		}
	...
	/**
	 * @param thumbnailUnsharp
	 *        the thumbnailUnsharp to set
	 */
	...
	/**
	 * @param unsharpThumbnailSizes
	 *        the unsharpThumbnailSizes to set
	 */
