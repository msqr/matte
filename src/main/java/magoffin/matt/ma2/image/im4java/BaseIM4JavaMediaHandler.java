 ...
 * <dd>The +profile setting to use for stripping out profile information from
 * processed thumbnail images. Defaults to <code>!icm,*</code> which strips out
 * all profiles except for the color (ICM) profile.</dd>
 * 
 * <dt>normalProfile</dt>
 * <dd>The +profile setting to use for stripping out profile information from
 * processed normal images. Defaults to <code>!icm,*</code> which strips out all
 * profiles except for the color (ICM) profile.</dd>
 * 
 * <dt>profileThumbnailSizes</dt>
 * <dd>A set of {@link MediaSize} for which to treat as thumbnail sizes and
 * profile with the <code>thumbnailProfile</code> as opposed to the
 * <code>normalProfile</code>. Defaults to a set containing
 * {@link MediaSize#THUMB_SMALL}, {@link MediaSize#THUMB_NORMAL}, and
 * {@link MediaSize#THUMB_BIG} and {@link MediaSize#THUMB_BIGGER}.</dd>
 * 
 * <dt>useSizeHint</dt>
 * <dd>If <em>true</em> then use the <code>-size</code> operator with a geometry
 * value double the desired output, for efficiently reading large files while
 * producing smaller copies of them. Defaults to <em>true</em>. This is the
 * parameter GraphicsMagick uses for this hint, as well as older ImageMagick
 * versions.</dd>
 * 
 * <dt>useJpegSizeHint</dt>
 * <dd>If <em>true</em> then use the <code>-define jpeg:size=</code> operator
 * with a geometry value bould the desired output, for efficiently reading large
 * JPEG files while producing smaller copies of them. This is the value newer
 * versions of ImageMagick use for this hint, but having it enabled for
 * GraphicsMagick should not cause any effect. Defaults to <em>true</em>.</dd>
 * </dl>
 *
 * @author matt
 * @version 1.1
...
public abstract class BaseIM4JavaMediaHandler extends BaseImageMediaHandler {
	...
	private Set<MediaSize> profileThumbnailSizes = EnumSet.of(MediaSize.THUMB_BIGGER,
	 ...
	 * {@link #setupBaseItemProperties(MediaItem, File)} followed by
	...
	protected void defaultHandleResource(MediaItem item, MediaRequest request, MediaResponse response,
		...
		try {
			...
			String profileValue = profileThumbnailSizes.contains(request.getSize()) ? thumbnailProfile
					: normalProfile;
			...
			if ( request.getParameters().containsKey(MediaRequest.OUTPUT_FILE_KEY)
				...
				// use temp file
				String extension = secondaryCommands.size() == 0 ? getFileExtension(item, request)
						: "miff";
			...
			for ( Iterator<ImageCommandAndOperation> cmdItr = secondaryCommands.iterator(); cmdItr
					.hasNext(); ) {
				...
				secondaryCmd.getCommand().run(secondaryCmd.getOp(), outFile.getAbsolutePath(),
				 ...
				 * File.createTempFile("IM4JavaTemp-encode-",
				 * "."+getFileExtension(item, request)); }
				 */
