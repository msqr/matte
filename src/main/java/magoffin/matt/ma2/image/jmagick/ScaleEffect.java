 ...
 * default the thumbnail algorithm is set to <b>triangle</b> and the
 ...
 * <dt>filterNameMap</dt>
 * <dd>A mapping of filter names to {@link FilterType} constant values. This map
 * is populated by default with each FilterType constant name, minus the
 * "Filter" ending. For example the {@link FilterType#BesselFilter} constant is
 * named <code>Bessel</code>.</dd>
 * 
...
public class ScaleEffect extends BaseJMagickMediaEffect {
	...
	private Set<MediaSize> thumbnailSizes = EnumSet.of(MediaSize.THUMB_BIG, MediaSize.THUMB_NORMAL,
	...
	public MagickImage applyEffect(MediaItem item, MediaRequest request, ImageInfo inInfo,
			MagickImage image) {
		String filterName = thumbnailSizes.contains(request.getSize()) ? thumbnailFilterName
				: normalFilterName;
		Integer filterType = filterNameMap.get(filterName);
		if ( filterType == null ) {
			throw new RuntimeException("The filter [" + filterName
					+ "] is not configured. Available names are: " + filterNameMap.keySet());
		}
