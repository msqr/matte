...
public abstract class GenericIndexableHibernateDao<T, PK extends Serializable>
				...
				public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
					...
					if ( callback.getStartDate() != null && callback.getEndDate() != null ) {
						...
						if ( log.isInfoEnabled() ) {
							log.info("Executing query to index leads with batch size [" + indexBatchSize
									+ "] and id > ["
									+ (callbackData.getId() == null ? "anything" : callbackData.getId())
									+ "] between [" + ymdFormat.format(callback.getStartDate())
									+ "] and [" + ymdFormat.format(callback.getEndDate())
									+ "] with SQL: " + sql);
						}
						...
						if ( callbackData.getId() != null ) {
							psmt.setObject(sqlParamIdxWithDateRange, callbackData.getId());
						}
					} else {
	 ...
	 * 
	 * <p>
	 * If the RowMapper returns a Map, that Map will be set on the
	 * BasicIndexData's mapRow property. Otherwise the returned object will be
	 * set on the mapRow Map under the key <code>rowObject</code>.
	 * </p>
	 * 
	 * @param rs
	...
	protected void populateIndexRow(ResultSet rs, int rowNum, BasicIndexData<PK> callbackData,
			RowMapper<?> rowMapper) throws SQLException {
		populateIndexDataId(callbackData, rs);
	...
	/**
	 * Populate the ID on a BasicIndexData instance from the current
	 * {@link ResultSet} row.
	 * 
	 * <p>
	 * Generally this will involve using the
	 * {@link #getIndexObjectIdColumnName()} column value.
	 * </p>
	 * 
	 * @param callbackData
	 *        the data to populate the ID on
	 * @param rs
	 *        the active ResultSet
	 * @throws SQLException
	 *         if an error occurs
	 */
	protected abstract void populateIndexDataId(BasicIndexData<PK> callbackData, ResultSet rs)
			throws SQLException;
	 ...
	 */
	public RowMapper<Map<String, Object>> getIndexRowMapper() {
		return indexRowMapper;
	}

	/**
	 * @param indexRowMapper
	 *        the indexRowMapper to set
	 */
	public void setIndexRowMapper(RowMapper<Map<String, Object>> indexRowMapper) {
	...
	/**
	 * @param sqlIndexDateRange
	 *        the sqlIndexDateRange to set
	 */
	...
	/**
	 * @param sqlParamIdxWithDateRange
	 *        the sqlParamIdxWithDateRange to set
	 */
	...
	/**
	 * @param indexObjectIdColumnName
	 *        the indexObjectIdColumnName to set
	 */
	...
	/**
	 * @param indexObjectTableAlias
	 *        the indexObjectTableAlias to set
	 */
